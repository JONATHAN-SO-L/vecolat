...
<header id="main-header">
 ...
 include ("conexi.php");
/*
if(isset($_POST['nom_largo'])&& !empty($_POST['nom_largo'])&&
		isset($_POST['nom_corto'])&& !empty($_POST['nom_corto'])&&
		isset($_POST['giro'])&& !empty($_POST['giro'])&&
		isset($_POST['rfc'])&& !empty($_POST['rfc'])&&
		isset($_POST['direccion'])&& !empty($_POST['direccion'])&&
		isset($_POST['entidad'])&& !empty($_POST['entidad'])&&
		isset($_POST['telefono'])&& !empty($_POST['telefono'])&&
		isset($_POST['pagina_web'])&& !empty($_POST['pagina_web'])&&
		isset($_POST['direccion_gps'])&& !empty($_POST['direccion_gps'])&&
		isset($_POST['observaciones'])&& !empty($_POST['observaciones'])){
			
			$con=mysqli_connect($host,$user,$pw,$db);
			mysqli_query($con,("INSERT INTO empresas(nombre_completo, nombre_corto, giro_comercial, rfc, direccion, entidad_federativa, telefono, pagina_web, direccion_gps, observaciones)VALUES('$_POST[nom_largo]','$_POST[nom_corto]','$_POST[giro]','$_POST[rfc]','$_POST[direccion]','$_POST[entidad]','$_POST[telefono]','$_POST[pagina_web]','$_POST[direccion_gps]','$_POST[observaciones]')"));
	...
	if(isset($_POST['rfc']) && isset($_POST['razon_social']) && isset($_POST['nombre_corto'])){ 
		...
		*/
		///////////////////////////////////////////////////////////////////////////////////////
		/*
		$formatos=array('.png','.jpg','.doc','.xlsx','.pdf');
	if(isset($_POST['boton'])){
		$nombreArchivo=$_FILES['archivo']['name'];
		$nombreTmpArchivo=$_FILES['archivo']['tmp_name'];
		$ext=substr($nombreArchivo, strrpos($nombreArchivo, '.'));
		if(in_array($ext, $formatos)){
			if(move_uploaded_file($nombreTmpArchivo, "empresas/$nombreArchivo")){
				echo"";
			}else{
				echo"";
			}
		...
		for ($i=0; $i <=$num_archivo; $i++){
			if(!empty($_FILES['archivo1']['name'][$i])){
				...
				$rnombre1=$_FILES['archivo1']['name'][$i];
		...
		/////////////////////////////////////////////////////////////////
		/////////////////////////////////////////////////////////////////
		$num_archivo2=count($_FILES['archivo2']['name']);
		for ($i=0; $i <=$num_archivo; $i++){
			if(!empty($_FILES['archivo2']['name'][$i])){
				
				$ruta_nueva2="files/empresas/$rfc"; //era rfc
		if(!file_exists($ruta_nueva2)){
						mkdir($ruta_nueva2);
					}
				$ruta_nueva2=$ruta_nueva2."/".$_FILES['archivo2']['name'][$i];
				$rnombre2=$_FILES['archivo2']['name'][$i];
				if(file_exists($ruta_nueva2)){
					echo "";
				}else{
		...
		for ($i=0; $i <=$num_archivo; $i++){
			if(!empty($_FILES['archivo3']['name'][$i])){
				...
				$ruta_nueva3="files/empresas/$rfc"; //era rfc
				$rnombre3=$_FILES['archivo3']['name'][$i];
      ...
      <div class="panel panel-success">
        ...
        <div class="panel-body">
            <form role="form" action="" method="POST" enctype="multipart/form-data">
						...
						<h3> Archivo 1</h3>
						<input type="file"  required=""  name="archivo1[]"/>
						</div>
						<div class="caja">
						<form method="post" action="" enctype="multipart/form-data">
						<h3> Archivo 2</h3>
