...
<section id="content">	 
  ...
  <table>
		    <a href="add_ubicacion.php" ><button type="submit" value="Adicionar" name="" class="btn btn-primary" style="text-align:center"><i class="fa fa-plus"></i>&nbsp;&nbsp;Adicionar</button></a>
			<td>
 <tr>
		...
		<div class="container" >
          <div class="row">
           ...
           <div class="page-header2">
                ...
                <h1 class="animated lightSpeedIn">Ubicación</h1>
...
<div class="buscar_index" style="width: 1100px;">	
    <?php
	$usuario = "veco_dvi";
	...
	inner join empresas on ubicacion.empresa_id=empresas.id
	inner join edificio on ubicacion.edificio_id=edificio.id_edificio order by id_ubicacion
	";
	$resultado = mysqli_query( $conexion, $consulta) or die ( "Algo ha ido mal en la consulta a la base de datos");



ECHO "<table class='table table-hover table-striped table-bordered'><th>Acción</th><th>Id</th><th>Empresa</th><th>Edificio</th><th>Descripción</th><th>Requisitos Acceso</th>";
while ($fila = mysqli_fetch_array( $resultado )){
	echo "<tr>";
      ?>
		<td class="text-center">
					<a href="edit_ubicacion.php?id=<?php echo $fila['id_ubicacion']; ?>" class="btn btn-sm btn-success"><i class="fa fa-pencil" aria-hidden="true"></i></a>
				    <a href="copy_ubicacion.php?id=<?php echo $fila['id_ubicacion']; ?>"class="btn btn-sm btn-warning"><i class="fa fa-clone" aria-hidden="true"></i></a>
					<a href="eliminar_ubicacion.php?id=<?php echo $fila['id_ubicacion']; ?>"class="btn btn-sm btn-danger"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
					<a href="ver_ubicacion.php?id=<?php echo $fila['id_ubicacion']; ?>" class="btn btn-info"><i class="fa fa-eye" aria-hidden="true"></i></a>
        </td>
        <?php
		ECHO " <TD>".utf8_decode($fila["id_ubicacion"])."</TD>";
        ECHO " <TD>".utf8_decode($fila["nombre_corto"])."</TD>";
        ECHO " <TD>".utf8_decode($fila["descripcion"])."</TD>";
		ECHO " <TD>".utf8_decode($fila["ubicacion"])."</TD>";
		ECHO " <TD>".utf8_decode($fila["requisitos_acc"])."</TD>";
