...
<div class="container">
  <div class="row">
	...
	$mysqli = new mysqli('localhost', "veco", "Vec83Dv19iSa@", 'veco_sims_devecchi');
			...
			$sqlA = $mysqli->query("SELECT clave FROM usuario WHERE nombre_usuario ='".$_SESSION['nombre']."' ");			
			$rowA = $sqlA ->fetch_array();
			
			if($rowA['clave'] == $passActual){
				if($pass1 == $pass2){
					$update = $mysqli->query("update usuario set clave = '$pass1' WHERE nombre_usuario ='".$_SESSION['nombre']."' ");
					if($update){
